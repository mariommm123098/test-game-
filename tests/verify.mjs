...
assert.ok(!game.includes('classList.add("scene--cutting")'), "Black-mask shot cut must not return");
assert.ok(game.includes("scene--camera-moving"), "Continuous camera movement controller missing");
assert.ok(css.includes("transform-origin 1.35s"), "Shot origin must animate smoothly");
